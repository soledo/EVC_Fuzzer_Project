"""
ServicePaymentSelectionRequest 메시지를 가지고 퍼징을 수행하는 코드입니다. 
타겟 상태머신 : Wait for ServicePaymentSelectionRequest
"""
...
class _SLACHandler:
    ...
    def start(self):
        ...
        # Neighbor Solicitation 응답을 처리하는 스니퍼 시작
        self.neighborSolicitationThread = AsyncSniffer(
            iface=self.iface, 
            lfilter=lambda x: x.haslayer("ICMPv6ND_NS") and x[ICMPv6ND_NS].tgt == self.sourceIP, 
    ...
    def handlePacket(self, pkt):
        # 이더넷 타입이 HomePlug AV가 아니거나 출발지가 자신인 경우 무시
        ...
        if pkt.haslayer("CM_SLAC_PARM_CNF"):
            ...
            # START_ATTEN_CHAR_IND 패킷 3개 생성 및 전송
            startSoundsPkts = [self.buildStartAttenCharInd() for _ in range(3)]
            # MNBC_SOUND_IND 패킷 numSounds개 생성 및 전송
    ...
    # START_ATTEN_CHAR_IND 패킷을 생성합니다
    ...
    def buildSetKeyReq(self):
        ethLayer = Ether()
        ethLayer.src = self.sourceMAC
        ethLayer.dst = "00:b0:52:00:00:01"  # 특정 AtherosC MAC 주소
...
class _TCPHandler:
    def __init__(self, pev: PEV, iterations_per_element):
        ...
        self.state = {}
        self.elements_to_modify = ["SelectedPaymentOption", "ServiceID"]  # 퍼징할 요소 목록
    ...
    def start(self):
        ...
        self.neighborSolicitationThread = AsyncSniffer(
            iface=self.iface, 
            lfilter=lambda x: x.haslayer("ICMPv6ND_NS") and x[ICMPv6ND_NS].tgt == self.sourceIP, 
            prn=self.sendNeighborAdvertisement
        )
        self.neighborSolicitationThread.start()
    ...
    def send_supported_app_protocol_request(self):
        ...
        if exi_payload is not None:
            try:
                ...
                # 실제 TCP 페이로드 길이 계산 (V2GTP 헤더 8바이트 포함)
                tcp_payload_length = len(exi_payload_bytes) + 8
                sendp(packet, iface=self.iface, verbose=0)
                self.seq += tcp_payload_length  # 시퀀스 번호 증가
        ...
        if self.supported_app_response_received.wait(timeout=15):
            ...
            if self.session_setup_response_received.wait(timeout=15):
                ...
                if self.service_discovery_response_received.wait(timeout=15):
                    print("INFO (TCPHandler): Received ServicePaymentSelectionResponse, starting fuzzing.")
                    self.send_fuzzing_messages()
                else:
                    print("WARNING (TCPHandler): ServicePaymentSelectionResponse not received within timeout, not starting fuzzing.")
            else:
    ...
    def send_fuzzing_messages(self):
        # 초기 XML 메시지 빌드 (ServicePaymentSelectionRequest 사용)
        handler = PacketHandler()
        handler.ServicePaymentSelectionRequest()
        xml_string = ET.tostring(handler.root, encoding='unicode')
